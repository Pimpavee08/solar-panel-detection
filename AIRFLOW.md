...
## ไม่ต้องเปิดหน้า Airflow เอง

สิ่งที่ปกติต้องเข้าไปดูใน <http://localhost:8080> ถูกดึงมาไว้ในหน้าเว็บของเราแล้ว
หน้า 8080 เหลือไว้สำหรับกรณีที่ต้องเจาะลึกจริง ๆ เท่านั้น

| อยากทำอะไร | ทำที่หน้าเว็บเราได้เลย |
|---|---|
| ดูว่าติดอยู่ step ไหน | stepper ในแท็บ "สร้างงานใหม่" |
| ดูว่าแต่ละ step ใช้เวลาเท่าไหร่ | ตัวเลขต่อท้ายชื่อ step |
| อ่าน log ของ step | กด `log` ข้างชื่อ step (เลือกดูได้ทีละครั้งที่ retry) |
| สั่งรันใหม่ทั้งงาน | ปุ่ม "สั่งรันใหม่ตั้งแต่ต้น" หรือปุ่ม "รันใหม่" ในการ์ดงาน |

`GET /tasks/{tid}/steps/{step}/log` เป็นตัวส่งต่อ log จาก Airflow โดยตรวจสิทธิ์เจ้าของ
ก่อนเสมอ เพราะ Airflow เองไม่รู้จักผู้ใช้เลย ส่วน `POST /tasks/{tid}/rerun` จะรีเซ็ต
Task_Step ทั้ง 4 แถวกลับเป็น pending ลบ Task_Result เดิม แล้วสั่ง dag run ใหม่
(ใช้ชื่อ run ที่ต่อท้ายด้วยเวลา เพราะชื่อเดิมจะชนกับ run ก่อนหน้า)
